...
// ── DB connection with retry (no healthcheck in docker-compose) ───────────────
let pool;
...
async function waitForDb(retries, delay) {
  for (var i = 0; i < retries; i++) {
    try {
      ...
      pool = null; // reset so it retries creating pool
    }
  }
...
async function enrichGameWithPlayerSlugs(game, conn) {
  ...
  return Object.assign({}, game, { player_slugs: ids.map(function(id) { return slugMap[id] || null; }) });
}
...
app.post("/api/auth/verify", function(req, res) {
  ...
  var actual = APP_PASSCODE;
  // Pad to equal length for timing-safe compare
  var len = Math.max(submitted.length, actual.length, 1);
  var a = Buffer.from(submitted.padEnd(len, "\0").slice(0, len));
  var b = Buffer.from(actual.padEnd(len, "\0").slice(0, len));
  var ok = crypto.timingSafeEqual(a, b) && submitted === actual;
  if (ok) {
...
app.put("/api/games/:gameId/rounds/:roundIndex", async function(req, res) {
  try {
    ...
    var entries = (req.body.entries || []).slice(0, 10).map(function(e) {
      return { bid: sanitizeStr(String(e.bid || "")), handsWon: sanitizeStr(String(e.handsWon || "")), gotBid: Boolean(e.gotBid) };
    });
...
// ── Start ─────────────────────────────────────────────────────────────────────
waitForDb(20, 3000).then(function() {
